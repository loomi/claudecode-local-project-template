...
API REST construída com **NestJS 11** + **Prisma** (SQLite por padrão ou PostgreSQL via `DATABASE_PROVIDER`) e documentada via **Swagger**.
...
```
HTTP → Controller → Service → PrismaService → SQLite (prisma/dev.db) ou PostgreSQL
                       ↑              ↓
